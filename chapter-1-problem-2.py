...
class StarCommand:
    def __init__(self, length, width=1,color="black"):
        self.length = length
        self.width = width
        self.color = color
    ...
    def __str__(self):
        return '<Command length="' + str(self.length) + '" width="' + \
               str(self.width) + '" color="' + self.color + '">Star</Command>'
...
class DrawingApplication(tkinter.Frame):
    ...
    def buildWindow(self):
        ...
        def parse(filename):
            ...
            for commandElement in graphicsCommands:
                ...
                if command == "GoTo":
                    ...
                    length = float(attr["length"].value)
                    width = float(attr["width"].value)
                    color = attr["color"].value.strip()
                    cmd = StarCommand(length,width,color)                
        ...
        starLengthSize = tkinter.StringVar()
        starWidthEntry = tkinter.Entry(sideBar,textvariable=starLengthSize)
        starWidthEntry.pack()
        starLengthSize.set(str(1))
        
        # A button widget calls an event handler when it is pressed. The starHandler
        # function below is the event handler when the Draw Star button is pressed. 
        def starHandler():
            ...
            cmd = StarCommand(int(starLengthSize.get()), int(widthSize.get()), penColor.get())
            cmd.draw(theTurtle)
            self.graphicsCommands.append(cmd)
