...
class LinkedList:
    ...
    def __delitem__(self,index):
        # This is left as an exercise for the reader. 
        if index >= 0 and index < self.numItems:
            cursor = self.first.getNext() # the node at index
            cursorPrev = self.first.getNext() # the node at index - 1
            for i in range(index):
                if i <= index - 2: 
                    cursorPrev = cursorPrev.getNext()
                cursor = cursor.getNext()
                
            cursorPrev.setNext(cursor.getNext())
            if index == self.numItems - 1: # set the last item in the list = node at index - 1 if node at index is last item
                self.last = cursorPrev
            self.numItems -= 1
            return 
